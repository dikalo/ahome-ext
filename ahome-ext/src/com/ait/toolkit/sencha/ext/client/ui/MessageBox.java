...
import com.ait.toolkit.sencha.ext.client.core.config.MessageBoxConfig;
import com.ait.toolkit.sencha.ext.client.events.messagebox.AlertCallback;
import com.ait.toolkit.sencha.ext.client.events.messagebox.ConfirmCallback;
import com.ait.toolkit.sencha.ext.client.events.messagebox.PromptCallback;

/**
 * Utility class for generating different styles of message boxes.
 * <p>
 * Note that the MessageBox is asynchronous. Unlike a regular JavaScript alert (which will halt browser execution), showing a MessageBox will not cause the code to stop. For this
 * reason, if you have code that should only run after some user feedback from the MessageBox, you must use a callback function.
 */
public class MessageBox {
	...
	/**
	 * 
	 * Displays a standard read-only message box with an OK button (comparable to the basic JavaScript Window.alert). If a callback function is passed it will be called after the
	 * user clicks the button, and the id of the button that was clicked will be passed as the only parameter to the callback (could also be the top-right close button).
	 * 
	 * @param title
	 *            the title
	 * @param message
	...
	public static native void alert(String title, String message, AlertCallback cb) /*-{
		$wnd.Ext.MessageBox
				.alert(
						...
						message,
						function() {
							cb.@com.ait.toolkit.sencha.ext.client.events.messagebox.AlertCallback::execute()();
						});
	}-*/;
	...
	/**
	 * Displays a confirmation message box with Yes and No buttons (comparable to JavaScript's Window.confirm). If a callback function is passed it will be called after the user
	 * clicks either button, and the id of the button that was clicked will be passed as the only parameter to the callback (could also be the top-right close button).
	 * 
	 * @param title
	 *            the title
	 * @param message
	...
	public static native void confirm(String title, String message, ConfirmCallback cb) /*-{
		$wnd.Ext.Msg
				.confirm(
						...
						message,
						function(btnID) {
							cb.@com.ait.toolkit.sencha.ext.client.events.messagebox.ConfirmCallback::execute(Ljava/lang/String;)(btnID);
						});
	}-*/;
	...
	/**
	 * Displays a message box with OK and Cancel buttons prompting the user to enter some text (comparable to JavaScript's Window.prompt). The prompt can be a single-line or
	 * multi-line textbox. If a callback function is passed it will be called after the user clicks either button, and the id of the button that was clicked (could also be the
				...
				.prompt(
						...
						function(btnID, text) {
							...
							if (text === undefined || text == '')
								text = null;
							cb.@com.ait.toolkit.sencha.ext.client.events.messagebox.PromptCallback::execute(Ljava/lang/String;Ljava/lang/String;)(btnID, text);
						});
	}-*/;
	...
	/**
	 * Adds the specified icon to the dialog. By default, the class 'ext-mb-icon' is applied for default styling, and the class passed in is expected to supply the background image
	 * url. Pass in empty string ('') to clear any existing icon. The following built-in icon classes are supported, but you can also pass in a custom class name:
	 * 
	 * <pre>
	 * 
	 * MessageBox.INFO
	...
	public static native void show(MessageBoxConfig config) /*-{
		$wnd.Ext.Msg.show(config.@com.ait.toolkit.core.client.JsObject::jsObj);
	}-*/;
