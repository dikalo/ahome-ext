...
import com.ait.toolkit.sencha.ext.client.events.form.BeforeDeselectHandler;
import com.ait.toolkit.sencha.ext.client.events.form.BeforeSelectHandler;
import com.ait.toolkit.sencha.shared.client.core.EventObject;
import com.ait.toolkit.sencha.shared.client.core.Template;
import com.ait.toolkit.sencha.shared.client.core.XTemplate;
import com.ait.toolkit.sencha.shared.client.data.Store;
import com.ait.toolkit.sencha.shared.client.dom.DomQuery;
 ...
 * <p>
 * A ComboBox is like a combination of a traditional HTML text &lt;input&gt; field and a &lt;select&gt; field; the user is able to type freely into the field, and/or pick values
 * from a dropdown selection list. The user can input any value by default, even if it does not appear in the selection list; to prevent free-form values and restrict them to items
 * in the list, set forceSelection to true.
 * <p>
 * The selection list's options are populated from any Ext.data.Store, including remote stores. The data items in the store are mapped to each option's displayed text and backing
 * value via the valueField and displayField configurations, respectively.
 * <p>
 * f your store is not remote, i.e. it depends only on local data and is loaded up front, you should be sure to set the queryMode to 'local', as this will improve responsiveness
 * for the user.
 */
public class ComboBox extends Picker {
	...
	private static native void init()/*-{
		...
		@com.ait.toolkit.sencha.ext.client.ui.ComboBox::configPrototype = c.initialConfig;
	}-*/;
	...
	public ComboBox(Map<String, String> values) {
		setQueryMode(LOCAL);
		setDisplayField("value");
		List<InternalModel> list = new ArrayList<InternalModel>();

		for (String label : values.keySet()) {
			list.add(new InternalModel(label, values.get(label)));
		}
	...
	private final static class InternalModel extends BaseModel {

		public InternalModel(String label, String value) {
			set("label", label);
	 ...
	 * @param forceAll
	 *            true to force the query to execute even if there are currently fewer characters in the field than the minimum specified by the minChars config option. It also
	 *            clears any filter previously saved in the current store (defaults to false)
	 */
	 ...
	 *            clears any filter previously saved in the current store (defaults to false)
	 * @param rawQuery
	 *            , Pass as true if the raw typed value is being used as the query string. This causes the resulting store load to leave the raw value undisturbed.(default to
	 *            false)
	 */
	...
	public native void select(BaseModel record) /*-{
		var cb = this.@com.ait.toolkit.sencha.ext.client.core.Component::getOrCreateJsObj()();
		cb.select(record.@com.ait.toolkit.data.client.BaseModel::getJsObj());
	}-*/;
	...
	/**
	 * Select an item in the dropdown list by its numeric index in the list. This function does NOT cause the select event to fire. The store must be loaded and the list expanded
	...
	/**
	 * Select an item in the dropdown list by its data value. This function does NOT cause the select event to fire. The store must be loaded and the list expanded for this
	...
	/**
	 * 
	 * true to automatically highlight the first result gathered by the data store in the dropdown list when it is opened. A false value would cause nothing in the list to be
	 * highlighted automatically, so the user would have to manually highlight an item before pressing the enter or tab key to select it (unless the value of (typeAhead) were
	 * true), or use the mouse to select a value.
	 * <p>
	 * Defaults to: true
	 */
	...
	/**
	 * If set to true, allows the combo field to hold more than one value at a time, and allows selecting multiple items from the dropdown list. The combo's text field will show
	 * all selected values separated by the delimiter.
	 * <p>
	 * Defaults to: false
	 */
	...
	/**
	 * The mode in which the ComboBox uses the configured Store. Acceptable values are remote and local
	 */
	...
	/**
	 * The id, DOM node or Ext.Element of an existing HTML <select> element to convert into a ComboBox. The target select's options will be used to build the options in the
	 * ComboBox dropdown; a configured store will take precedence over this.
	 */
	...
	/**
	 * A CSS selector in any format supported by {@link DomQuery} that will be used to determine what nodes this DataView will be working with.
	 * 
	 * @param itemSelector
	 *            the items selector
	 */
	...
	/**
	 * Set to {@link com.ait.toolkit.sencha.ext.client.ui.ComboBox#LOCAL} if the ComboBox loads local data. (defaults to
	 * {@link com.ait.toolkit.sencha.ext.client.ui.ComboBox#REMOTE} which loads from the server)
	 * 
	 * @param mode
	...
	/**
	 * If greater than 0, a paging toolbar is displayed in the footer of the dropdown list and the filter queries will execute with page start and limit parameters. Only applies
	 * when mode = 'remote' (defaults to 0).
	 * 
	 * @param pageSize
	 *            the page size
	 * @see #setMode(com.ait.toolkit.sencha.ext.client.ui.ComboBox.Mode)
	 * @throws IllegalStateException
	 *             this property cannot be changed after the Component has been rendered
	 */
	...
	/**
	 * The length of time in milliseconds to delay between the start of typing and sending the query to filter the dropdown list (defaults to 500 if mode = 'remote' or 10 if mode =
	 * 'local')
	 * 
	 * @param queryDelay
	...
	/**
	 * Set the template to use to render the ComboBox items. Used to customize dispaly of ComboBox items. <br/>
	 * <br/>
	 * For example the template below uses the fields 'url' and 'text' from the Store's RecordDef. <br/>
	 * <br/>
	...
	/**
	 * The underlying data value name to bind to this CombBox (defaults to undefined). Note: use of a valueField requires the user make a selection in order for a value to be
	 * mapped.
	 * 
	 * @param valueField
	 *            the value field
	...
	/**
	 * When using a name/value combo, if the value passed to setValue is not found in the store, valueNotFoundText will be displayed as the field text if defined (defaults to
	 * undefined).
	 * 
	...
	public native HandlerRegistration addBeforeDeselectHandler(BeforeDeselectHandler handler)/*-{
		...
		var fn = function(c, r, i, e) {
			var combo = @com.ait.toolkit.sencha.ext.client.ui.ComboBox::new(Lcom/google/gwt/core/client/JavaScriptObject;)(c);
			var model = @com.ait.toolkit.data.client.BaseModel::new(Lcom/google/gwt/core/client/JavaScriptObject;)(r);
			var event = @com.ait.toolkit.sencha.ext.client.events.form.BeforeDeselectEvent::new(Lcom/ait/toolkit/sencha/ext/client/ui/ComboBox;Lcom/ait/toolkit/data/client/BaseModel;ILcom/google/gwt/core/client/JavaScriptObject;)(cmp,model,index,e);
			handler.@com.ait.toolkit.sencha.ext.client.events.form.BeforeDeselectHandler::onBeforeDeselect(Lcom/ait/toolkit/sencha/ext/client/events/form/BeforeDeselectEvent;)(event);
		};
		var eventName = @com.ait.toolkit.sencha.ext.client.events.form.BeforeDeselectEvent::EVENT_NAME;
	...
	public native HandlerRegistration addBeforeSelectHandler(BeforeSelectHandler handler)/*-{
		...
		var fn = function(c, r, i, e) {
			var combo = @com.ait.toolkit.sencha.ext.client.ui.ComboBox::new(Lcom/google/gwt/core/client/JavaScriptObject;)(c);
			var model = @com.ait.toolkit.data.client.BaseModel::new(Lcom/google/gwt/core/client/JavaScriptObject;)(r);
			var event = @com.ait.toolkit.sencha.ext.client.events.form.BeforeSelectEvent::new(Lcom/ait/toolkit/sencha/ext/client/ui/ComboBox;Lcom/ait/toolkit/data/client/BaseModel;ILcom/google/gwt/core/client/JavaScriptObject;)(cmp,model,index,e);
			handler.@com.ait.toolkit.sencha.ext.client.events.form.BeforeSelectHandler::onBeforeSelect(Lcom/ait/toolkit/sencha/ext/client/events/form/BeforeSelectEvent;)(event);
		};
		var eventName = @com.ait.toolkit.sencha.ext.client.events.form.BeforeSelectEvent::EVENT_NAME;
		component.addListener(eventName, fn);
		var toReturn = @com.ait.toolkit.sencha.ext.client.events.HandlerRegistration::new(Lcom/ait/toolkit/sencha/ext/client/core/Component;Ljava/lang/String;Lcom/google/gwt/core/client/JavaScriptObject;)(this,eventName,fn);
		return toReturn;
	}-*/;
	 ...
	 */
	public native HandlerRegistration addBeforeQueryHandler(com.ait.toolkit.sencha.ext.client.events.form.BeforeQueryHandler handler)/*-{
		var component = this.@com.ait.toolkit.sencha.ext.client.core.Component::getOrCreateJsObj()();
		var fn = function(queryPlan, e) {
			var event = @com.ait.toolkit.sencha.ext.client.events.form.BeforeQueryEvent::new(Ljava/lang/String;Lcom/google/gwt/core/client/JavaScriptObject;)(queryPlan, e);
			handler.@com.ait.toolkit.sencha.ext.client.events.form.BeforeQueryHandler::onBeforeQuery(Lcom/ait/toolkit/sencha/ext/client/events/form/BeforeQueryEvent;)(event);
		};
		var eventName = @com.ait.toolkit.sencha.ext.client.events.form.BeforeQueryEvent::EVENT_NAME;
		component.addListener(eventName, fn);
		var toReturn = @com.ait.toolkit.sencha.ext.client.events.HandlerRegistration::new(Lcom/ait/toolkit/sencha/ext/client/core/Component;Ljava/lang/String;Lcom/google/gwt/core/client/JavaScriptObject;)(this,eventName,fn);
		return toReturn;
	}-*/;
	 ...
	 */
	public native HandlerRegistration addSelectHandler(com.ait.toolkit.sencha.ext.client.events.form.SelectHandler handler)/*-{
		var component = this.@com.ait.toolkit.sencha.ext.client.core.Component::getOrCreateJsObj()();
		var fn = function(c, rec, e) {
			var combo = @com.ait.toolkit.sencha.ext.client.ui.ComboBox::new(Lcom/google/gwt/core/client/JavaScriptObject;)(c);
			var selection = @com.ait.toolkit.data.client.BaseModel::from(Lcom/google/gwt/core/client/JavaScriptObject;)(rec);
			var event = @com.ait.toolkit.sencha.ext.client.events.form.SelectEvent::new(Lcom/ait/toolkit/sencha/ext/client/ui/ComboBox;Lcom/ait/toolkit/data/client/BaseModel;Lcom/google/gwt/core/client/JavaScriptObject;)(combo,selection,e);
			handler.@com.ait.toolkit.sencha.ext.client.events.form.SelectHandler::onSelect(Lcom/ait/toolkit/sencha/ext/client/events/form/SelectEvent;)(event);
		};
		var eventName = @com.ait.toolkit.sencha.ext.client.events.form.SelectEvent::EVENT_NAME;
		component.addListener(eventName, fn);
		var toReturn = @com.ait.toolkit.sencha.ext.client.events.HandlerRegistration::new(Lcom/ait/toolkit/sencha/ext/client/core/Component;Ljava/lang/String;Lcom/google/gwt/core/client/JavaScriptObject;)(this,eventName,fn);
		return toReturn;
	}-*/;
